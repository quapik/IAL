...
*/
 ...
 {
 tQueue->f_index=0; //zacatek a konec fronty 0
 tQueue->b_index=0;
for(int i=0;i<QUEUE_SIZE;i++) //TODO MAX_QUEUE??
////////////////////////////////////////////////
	{
		tQueue->arr[i]='*';
	}
...
*/
	 ...
	 return (index+1)%QUEUE_SIZE; //zbytek po celociselnem deleni (vzdy neco <Queue_size)
...
*/
	if(tQueue->f_index==tQueue->b_index) //kdyz se rovnaji je fronta prazdna
	 {
		 return 1;
	 }
	 else
...
*/
if(nextIndex(tQueue->b_index)==tQueue->f_index) //pokud za b je f tak je plno
{return 0;}
else
{
return 1;
...
if(queueEmpty(q))
{
	queueEmpty(QERR_FRONT);
}
else
{
	*c=tQueue->arr[f_index];
}
}
...
*/
	...
	if(queueEmpty(q))
{
	queueEmpty(QERR_REMOVE);
}
else
{
	q->f_index=(nextIndex(q->f_index)); //pouze prepis prvniho prvku
}
}
...
{
	queueFront(q,c); //do c ulozi prvni prvek
	queueRemove(q);  //a první posune na dalsi
}
}
...
*/
	...
	if(queueFull(q)) //plna fronta
	{
		queueError(QERR_UP);
	}
	else
	{
		q->arr[q->b_index]=c; //na prvni volnou pozici ulozime a posuneme ji
